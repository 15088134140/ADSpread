export default {
  id: 'ID',
  confirm: 'Confirm',
  cancel: 'Cancel',
  save: 'Save',
  delete: 'Delete',
  edit: 'Edit',
  create: 'Create',
  add: 'Add',
  remove: 'Remove',
  search: 'Search',
  query: 'Search',
  reset: 'Reset',
  refresh: 'Refresh',
  operation: 'Action',
  status: 'Status',
  enabled: 'Enabled',
  disabled: 'Disabled',
  loading: 'Loading...',
  noData: 'No data',
  neverOnline: 'Never online',
  success: 'Success',
  failure: 'Failure',
  tip: 'Tip',
  all: 'All',
  yes: 'Yes',
  no: 'No',
  total: 'Total',
  unit: 'records',
  second: 's',
  permanent: 'Permanent',
  backHome: 'Back to Home',
  login: 'Login',
  logout: 'Logout',
  systemTitle: 'ADSpread Admin System',
  defaultAccountHint: 'Default account: admin / admin123',
  language: 'Language',
  profile: 'Profile',
  settings: 'Settings',
  confirmButtonText: 'Confirm',
  cancelButtonText: 'Cancel',
  networkError: 'Network error, please check your network connection',
  sessionExpired: 'Session expired, please log in again',
  forbidden: 'You do not have permission to access this resource',
  notFoundResource: 'The requested resource does not exist',
  serverError: 'Internal server error',
  requestFailed: 'Request failed',
  pageNotFound: 'Sorry, the page you visited does not exist',
  messages: {
    createSuccess: 'Created successfully',
    updateSuccess: 'Updated successfully',
    deleteSuccess: 'Deleted successfully',
    saveSuccess: 'Saved successfully',
    operateSuccess: 'Operation succeeded',
    operateFailed: 'Operation failed',
    loginSuccess: 'Login successful',
    loginFailed: 'Login failed',
    logoutSuccess: 'Logout successful',
    uploadSuccess: 'Upload successful',
    uploadFailed: 'Upload failed',
    publishSuccess: 'Published successfully',
    pushSuccess: 'Push successful',
    batchPushCompleted: 'Batch push completed',
    confirmDelete: 'Are you sure you want to delete this item?',
    confirmLogout: 'Are you sure you want to log out?',
    noPermission: 'No permission',
  },
  dashboard: {
    storeCount: 'Total Stores',
    deviceCount: 'Total Devices',
    materialCount: 'Total Materials',
    onlineDeviceCount: 'Online Devices',
    recentOperations: 'Recent Operations',
    quickActions: 'Quick Actions',
    operator: 'Operator',
    operationContent: 'Operation',
    durationMs: 'Duration (ms)',
    time: 'Time',
    // Section A — Core metric cards
    deviceOnlineRate: 'Device Online Rate',
    online: 'Online',
    offline: 'Offline',
    unbound: 'Unbound',
    pendingMaterial: 'Pending Materials',
    approved: 'Approved',
    rejected: 'Rejected',
    activePublishPlan: 'Active Publish Plans',
    inactive: 'Inactive',
    pushSuccessRate: 'Push Success Rate (7d)',
    successCount: 'Success',
    failureCount: 'Failure',
    // Section B — Content funnel
    contentFunnel: 'Content Funnel',
    materialStage: 'Material',
    programStage: 'Program',
    publishPlanStage: 'Publish Plan',
    pushStage: 'Push',
    pending: 'Pending',
    draft: 'Draft',
    published: 'Published',
    active: 'Active',
    // Section C — Operation todo
    operationTodo: 'Operation Todo',
    noTodo: 'No pending tasks',
    pushFail: 'Push Failures',
    unboundDevice: 'Unbound Devices',
    // Section D — Recent operation logs
    recentLogs: 'Recent Operation Logs',
    viewAll: 'View All',
    // Loading / error states
    loadFailed: 'Failed to load',
  },
  pagination: {
    total: 'Total {total} records',
  },
};
